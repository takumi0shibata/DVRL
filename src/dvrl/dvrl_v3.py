...
class Dvrl:
    ...
    def __init__(
        ...
        self.data_dim = self.prompt_data['prompt_embedding'].shape[1]
    ...
    def train_dvrl(self, metric: str = 'qwk') -> None:
        ...
        cluster, indices = np.unique(self.prompt_data['cluster'][self.prompt_data['essay_set'] != self.target_prompt_id], return_index=True)
        x_cluster = self.prompt_data['prompt_embedding'][self.prompt_data['essay_set'] != self.target_prompt_id][indices]
        x_cluster = torch.tensor(x_cluster, dtype=torch.float32).to(self.device)
        y_cluster = self.prompt_data['score'][self.prompt_data['essay_set'] != self.target_prompt_id][indices]
        ...
        for iteration in tqdm(range(self.outer_iterations), desc='Training DVRL'):
            ...
            est_dv_curr = self.value_estimator(x_cluster, y_cluster, y_pred_diff_by_cluster).squeeze()
