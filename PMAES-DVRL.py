...
if __name__ == '__main__':
    ...
    parser.add_argument('--run_name', type=str, default='PMAES-DVRL-nodev', help='name of the experiment')
    ...
    data_value_path = 'outputs/Estimated_Data_Values/Feature/'
    ...
    for p in np.arange(0.1, 1.0, 0.1):
        ...
        train_prompt_ids_tmp = train_prompt_ids[weights]
        train_essay_pos_tmp = train_essay_pos[weights]
        train_linguistic_tmp = train_linguistic[weights]
        train_readability_tmp = train_readability[weights]
        train_score_tmp = train_score[weights]
        ...
        print('s_batch_size: {}'.format(s_batch_size))
        print('t_batch_size: {}'.format(t_batch_size))
        tr_s_loader = DataLoader(PMAESDataSet(train_prompt_ids_tmp, train_essay_pos_tmp, train_linguistic_tmp, train_readability_tmp, train_score_tmp), batch_size=s_batch_size)
        ...
        for e_index in range(1, epochs+1):
            ...
            va_loss_high, te_loss_high, va_qwk_high, te_qwk_high, best_qwk_high = TrainSingleOverallScoring(args,
                                                                                    essay_encoder, scorer, pm_cl, optims,
                                                                                    tr_s_loader, va_s_loader, te_t_loader,
                                                                                    args.target_id, e_index,
                                                                                    tr_log, args.attribute_name
                                                                                    )
            if va_loss_high < best_loss_high:
                best_loss_high = va_loss_high
                best_dev_qwk_high = va_qwk_high
        ...
        for e_index in range(1, epochs+1):
            ...
            va_loss_low, te_loss_low, va_qwk_low, te_qwk_low, best_qwk_low = TrainSingleOverallScoring(args,
                                                                                    essay_encoder, scorer, pm_cl, optims,
                                                                                    tr_s_loader, va_s_loader, te_t_loader,
                                                                                    args.target_id, e_index,
                                                                                    tr_log, args.attribute_name
                                                                                    )
            if va_loss_low < best_loss_low:
                best_loss_low = va_loss_low
                best_dev_qwk_low = va_qwk_low
